...
with tarfile.open(args.json) as tar:
    mibig = {}
    for entry in filter(lambda entry: entry.name.endswith(".json"), iter(tar.next, None)):
        entry = json.load(tar.extractfile(entry))
        if entry["cluster"]["status"] == "active":
            mibig[entry["cluster"]["mibig_accession"]] = entry["cluster"]
print("Loaded {} active BGCs".format(len(mibig)))
...
# --- Map unannotated compounds to NPAtlas -----------------------------------
np_atlas_inchikeys = {
...
with open(args.output, "w") as dst:
    json.dump(mibig, dst, sort_keys=True, indent=4)      
